...
document.getElementById('history_list').innerHTML = '您已清空最近浏览过的商品';
  ...
  <div class="AreaR">
          ...
          <c:otherwise>
                <c:forEach var="item" items="${newsList}" varStatus="status">
                    <div class="goodsItem" style="padding: 10px 4px 15px 1px;">
                       <a href="${item.viewUrl}"><img src="/file/${item.documentDataMap.productSmallImage.dataValue}" alt="${item.title}" class="goodsimg" /></a><br />
                       <p><a href="${item.viewUrl}" title="">${item.title}</a></p>
                        本店价<font class="shop_s">￥
                            ${newsListPrice[status.index] }
                        元</font><br />
                    </div>
                </c:forEach>
          </c:otherwise>
    ...
    <div id="pager" class="pagebar" style="margin-top: 320px;">
        <span class="f_l " style="margin-right:10px;">总计 <b>${fn:length(newsList)}</b>  个记录</span>
      </div>
