    ...
    <div class="container-fluid">
	  ...
	  <%@include file="/WEB-INF/jsp/common/include/menu.jsp"%>
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<h2 class="sub-header"><span>添加支付通道</span></h2>
			<div class="table-responsive">	
				 ...
				 <table class="table table-striped" style="">
					...
					<tbody>
						...
						<tr>
							...
							<td style="text-align:left;">
								<input name="payChannelId" id="payChannelId"/>
							</td>							
						</tr>
						<tr>
							<td style="text-align:right;width:20%;min-width:250px;">对应支付类型</td>
							<td style="text-align:left;">
								<select name="payTypeId" id="payTypeId">					 
									<c:forEach var="i" items="${payType}" >
										<option value="${i.payTypeId}">${i.name}</option>
									</c:forEach>
								</select>
							</td>							
						</tr>
						<tr>
							...
							<td style="text-align:left;">
								<select name="processClass" id="processClass">			<c:forEach var="i" items="${process}" >
						...
						<tr>
							...
							<td style="text-align:left;">
								<input name="commission" id="commission"/>
							</td>							
						</tr>	
						<tr>
							<td  style="text-align:right;">系统资金账户ID</td>
							<td style="text-align:left;">
								<input name="referUuid" id="referUuid"/>
							</td>							
						</tr>	
						<tr>
						...
						<tr>
							<td  style="text-align:right;">相同权重下跳转比例</td>
							<td style="text-align:left;">
								<input name="percent" id="percent"/>%
							</td>							
						</tr>
						...
						<tr>
							...
							<td style="text-align:left;">
								<input name="${a.dataCode}" id="${a.dataCode}"/>
							</td>							
						</tr>	
						</c:forEach>
				...
				<div class="submit"><input type="submit" value="新增支付通道" class="submitinput"></div>
				</form>
			</div>
        </div>
      </div>
	...
	<script>
		...
		$(".submitinput").on('click',function(e){
				...
				if (payChannelId=="") {
					alert('请输入相关支付通道ID！');
					$('#payChannelId').focus();
				}else if(!reg1.test(payChannelId)){
					alert("请输入正确格式相关支付通道ID");
					$('#payChannelId').focus();
				}else if(name==""){
					alert('请输入名称！');
					$('#name').focus();
					...
					alert("请输入相同权重下跳转比例");
					$('#percent').focus();
				}else if(!reg.test(percent)){
					alert("请输入正确格式相同权重下跳转比例");
					$('#percent').focus();
				}else{
