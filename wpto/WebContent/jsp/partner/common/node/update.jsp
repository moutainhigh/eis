...
<html lang="zh-CN">
  ...
  <body>
	...
	<form:form id="nodeForm" commandName="node" name="nodeForm" action="/node/update.shtml">
		<form:input path="nodeId" type="hidden"/>
		<form:errors path="nodeId" cssClass="errorMessage"/>
		...
		<tr>
			...
			<td>
				...
				<c:forEach var="p" items="${nodeProcessorList}">
					<c:choose>
						<c:when test="${node.processClass== p}">
							<option id="processor" name="processor" value="${p}" selected="selected"><spring:message code="NodeProcessor.${p}" /></option>
						</c:when>
						<c:when test="${node.processClass!= p}">
		...
		<tr>
			...
			<td>
				...
				<c:forEach var="t" items="${templateList}">
					<c:choose>
						<c:when test="${node.templateId == t.templateId}">
							<option id="templateId" name="templateId" value="${t.templateId}" selected="selected">${t.templateName}[${t.templateId}]</option>
						</c:when>
						<c:when test="${node.templateId != t.templateId}">
		...
		<tr>
			...
			<td style="text-align:left;">
					...
					<ul id="browser" class="filetree" style="background:#F9F9F9; margin-left:40px;">
		...
		<tr>
			...
			<td>
				...
				<c:forEach var="t" items="${siteCodeList}">
					<c:choose>
						<c:when test="${node.siteCode == t}">
							<option id="siteCode" name="siteCode" value="${node.siteCode}" selected="selected">${node.siteCode}</option>
						</c:when>
						<c:when test="${node.siteCode != t}">
		...
		<tr>
			...
			<td>
				...
				<c:forEach var="status" items="${statusCodeList}">
					<c:choose>
						<c:when test="${node.currentStatus == status.id}">
							<option id="currentStatus"  value="${status.id}" selected="selected" ><spring:message code="Status.${status.id}" text="${status.name}"/></option>
						</c:when>
						<c:when test="${node.currentStatus != status.id}">
							<option id="currentStatus"  value="${status.id}"><spring:message code="Status.${status.id}" text="${status.name}"/></option>
						</c:when>
					</c:choose>
				</c:forEach>
				</select>
		...
		<tr id="lastTr"> <td align="center" colspan="2"><input class="querenbtn" type="submit" value="确认" /></td></tr>
		</table>
	</form:form>
</div>
        </div>
	...
	<script>
	var url="/node/update.json";
	$(function(){
		$(".querenbtn").on('click',function(e){
				e.preventDefault();
				

				$('span.subNode').each(function(){
						$(this).addClass('spannodeall');
					})
					$('span.parentNode').each(function(){
						$(this).addClass('spannodeall');
					})
					$('.spannodeall').each(function(){
						var index = $(this).index();
						var inputchek = $(this).find("input[name='includeNodeSet.nodeId']");
						var selcetxz = $(this).find("select[name='includeNodeSet.contentType'] option:selected").attr('value');
						var inputtext = $(this).find("input[name='includeNodeSet.rows']").val();
						if (inputchek.is(':checked') || selcetxz !=='' || inputtext !=='') {

							if(selcetxz==''){
								if (inputchek.is(':checked') || inputtext !=='') {
								$(this).find("select[name='includeNodeSet.contentType'] option:selected").attr('value','0');
							};  
							};
							$(this).addClass('unfinished');
							$(this).removeClass('kongzhi');
							if (inputchek.is(':checked')) {
								inputchek.removeClass('cheinput');
							}else{
								inputchek.addClass('cheinput');
							};
						}else{
							//$(this).addClass('kongzhi');
					...
					if ($('.kongzhi').length==$('.spannodeall').length){
						alert('子栏目不能为空！');
						$('.spannodeall').eq(0).find("input[name='includeNodeSet.nodeId']").focus();
					}else if($('.unfinished').length && $('.unfinished').length>0){
							var input1 = $('.unfinished').eq(0).find("input[name='includeNodeSet.nodeId']");
							var select2 = $('.unfinished').eq(0).find("select[name='includeNodeSet.contentType'] option:selected");

							var input3 = $('.unfinished').eq(0).find("input[name='includeNodeSet.rows']");
							if (input1.hasClass('cheinput')) {
								alert('子栏目不能为空！');
								input1.focus();
							}else if(input3.val()==''){
								alert('获取文章数量不能为空！');
						...
						var data = $("#nodeForm").serializeArray();
						var node = $.grep(data,function(item,index,input){
							if (item.value !== '') {
									return item;
							};
						})
