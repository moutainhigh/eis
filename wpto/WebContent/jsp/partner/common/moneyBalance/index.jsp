...
<body>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				<h2 class="sub-header"><span>资金平衡表</span></h2>
				<c:if test="${!empty addUrl}">
					<p><span class="createmoney addmoban" >生成资金平衡</span></p>
				</c:if>
						...
						<tbody>
							<c:forEach var="i" items="${rows}">
								<tr>
									...
									<td>${i.receivableMoney}</td>
									<td>${i.waitingSettlementMoney}</td>									
									<td>${i.arrivedMoney}</td>
									<td>${i.waitingWithdrawMoney}</td>									
									<td>${i.arrivedMoney - i.waitingWithdrawMoney}</td>									
									<td>${i.profit}</td>									
								</tr>
							</c:forEach>
						</tbody>
	...
	<script>
		...
		// 生成资金平衡
		$('.createmoney').on('click',function(){
			$.ajax({
				type:"POST",
				url:"/moneyBalance/create.json",
				dataType:"json",
				success:function(data){
					swal(data.message.message);
				}
