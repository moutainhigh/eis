...
#queryForm select, #queryForm input[type='text']{
  ...
  <body>
    ...
    <div class="container-fluid">
	  ...
	  <%@include file="/WEB-INF/jsp/common/include/menu.jsp"%>
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<h2 class="sub-header"><span>菜单列表</span></h2>
						   ...
						   <table width="100%"  >
							<tr>  
								<td>
									...
									<input type="text" class="calendarInput"  name="menuName" value="<%=request.getParameter("menuName")==null?"":request.getParameter("menuName")%>"/>
								</td>
								<td>
			...
			<div class="table-responsive">	
				 <table class="table table-striped">
		              ...
		              <% 
	                String menuNameV=request.getParameter("menuName");  
	                request.setAttribute("menuNameV",menuNameV); 
	                String menuUrlV=request.getParameter("menuUrl");  
	                request.setAttribute("menuUrlV",menuUrlV); 
	                %>
		              <c:if test="${menuNameV==null && menuUrlV==null || menuUrlV=='' && menuNameV==''}">
					<tbody class="menuall">
					<c:forEach var="i" items="${menu}">
						...
						<td style="position: relative;"><span class="tools" style="right: 5px; cursor: pointer;"><img src="/theme/basic/images/tools.png"></span>
							<ul style="position: absolute; width: 57px; padding-right: 10px; padding-left: 10px;left: 50%; margin-left: -27px; top: 30px;line-height: 26px; list-style: none; z-index: 100;  border: 1px solid rgb(221, 221, 221);  border-radius: 5px; background-color: rgb(255, 255, 255);display: none;" class="toolbtns">
								<li><a href="<c:url value='/partnerMenu/get/${i.menuId}.shtml'/>" class="updatePage">修改</a></li>
							</ul>
						</td>
						</tr>
					</c:forEach>
				...
				<tbody class="menusing">
					...
					<c:if test="${menuNameV==i.menuName || menuUrlV==i.menuUrl}">
						<tr class="trbody">
	                    <td>${i.menuId}</td>
	                    <td>${i.menuName}</td>
						<td><a href="<c:url value='${i.menuUrl}'/>"><c:url value='${i.menuUrl}'/></a></td>
	...
	<div class="increase addincrease" style="display:none;">
		<div class="increasebox">
			...
			<form action="" id="addpartnerForm">
				<p><span class="tdtitle">菜单名称：</span><input class="bigwidth calendarInput" name='menuName' type="text"  /></p>
				...
				<p><span class="tdtitle">对应资源：</span><input class="bigwidth calendarInput"  name='resourceId' type="text" placeholder="显示图标等" /></p>
	...
	<!-- <ul id="menuSearch" style="display: none;">
		<c:forEach var="a" items="${menuList}">
		<li data-url="${a.menuUrl}">${a.menuName}</li>
		</c:forEach>
	</ul> -->
