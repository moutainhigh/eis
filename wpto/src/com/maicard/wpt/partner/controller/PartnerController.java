...
@Controller
@RequestMapping(value={"/partner","/channel"})
public class PartnerController extends BaseController {
	...
	public String list(HttpServletRequest request, HttpServletResponse response, ModelMap map, 
			@ModelAttribute("partnerCriteria")UserCriteria partnerCriteria) throws Exception {
		final String view = "common/partner/list";
		...
		//检查当前请求是partner还是channel
		String uri = request.getRequestURI();
		
		String modelType = uri.split("/")[1].replaceAll("\\.\\w*$", "");
		//logger.info("当前URL:" + uri + ",解析为请求:" + modelType);

		if(authorizeService.havePrivilege(partner, ObjectType.partner.name(), "w")){
			map.put("addUrl", modelType + "/create.shtml" );
		}
		
		//对于当前请求为/channel的，只允许查询extraType>0的用户类型，不允许查询=0的内部用户
		if(modelType.equalsIgnoreCase("channel") && partnerCriteria.getUserExtraTypeId() <= 0){
				partnerCriteria.setUserExtraTypeId(UserExtraType.accountChargeSupplier.id);
		}
	...
	public String get(HttpServletRequest request, HttpServletResponse response, ModelMap map,
		...
		if (dataDefineList == null || dataDefineList.size() < 1) {
			...
			for(DataDefine dataDefine : dataDefineList){
				if(dataDefine.getInputLevel() == null || dataDefine.getInputLevel().equals(InputLevel.user.name())){
		...
		//对于当前请求为/channel的，只允许查询extraType>0的用户类型，不允许查询=0的内部用户
		if(modelType.equalsIgnoreCase("channel") && selectUser.getUserExtraTypeId() <= 0){
	...
	public String detailSelf(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		String view = "common/partner/detail";
	...
	public String getUpdate(HttpServletRequest request, HttpServletResponse response, ModelMap map,@PathVariable("uuid")int uuid) throws Exception {
		...
		List<UserRoleRelation> partnerRoleList = partnerRoleRelationService.list(partnerRoleRelationCriteria);
		map.put("statusCodeList", SecurityStandard.UserStatus.values());
		map.put("userExtraTypeList",UserExtraType.values());
	...
	public String getConfig(HttpServletRequest request, HttpServletResponse response, ModelMap map,@PathVariable("uuid")long uuid) throws Exception {
		...
		final String view = "common/partner/config";
