	...
	public String detail(HttpServletRequest request, HttpServletResponse response, ModelMap map,
		...
		/*if (log != null && log.size() > 0) {
			for (Item logs : log) {
				...
				if (TransactionStatus.unknown.findById(logs.getCurrentStatus()).getName() != "未知") {
					logs.setCurrentStatusName(TransactionStatus.unknown.findById(logs.getCurrentStatus()).getName());
				} else {
					logs.setCurrentStatusName(EisError.unknown.findById(logs.getCurrentStatus()).getName());
				}
