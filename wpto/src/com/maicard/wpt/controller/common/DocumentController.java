...
public class DocumentController extends BaseController {
	...
	public String getCreate(HttpServletRequest request, HttpServletResponse response, ModelMap map, DocumentCriteria documentCriteria) throws Exception {
		...
		if (documentTypeId > 0) {
			documentType = documentTypeService.select(documentTypeId, DataFetchMode.full.toString());
		}
		...
		DocumentTypeCriteria documentTypeCriteria = new DocumentTypeCriteria();
		map.put("documentTypeList", documentTypeService.list(documentTypeCriteria));
		...
		List<DocumentType> documentTypeList2 = new ArrayList<DocumentType>();
		/*
		 * 忽略掉产品文档的类型，因为该类型已转移到产品管理, NetSnake,2016-07-09
		 */
		/*if(documentTypeList != null && documentTypeList.size() > 0){
			for(DocumentType dt : documentTypeList){
				if(dt.getDocumentTypeCode().equals(ObjectType.product.name())){
					continue;
				}
				documentTypeList2.add(dt);
			}
		}*/
		...
		logger.debug("创建文档使用" +  workflowInstance.getWorkflowInstanceId() + "#工作流实例");
	...
	public String create(HttpServletRequest request, HttpServletResponse response, ModelMap map, Document document) throws Exception {
		...
		document.setOwnerId(frontUser.getOwnerId());
		document.setPublisherId(frontUser.getUuid());
		...
		if(defaultNode.getOwnerId() != frontUser.getOwnerId()){
			logger.error("指定的默认发布节点[" + defaultNode.getNodeId() + "]的ownerId[" + defaultNode.getOwnerId() + "]与系统用户的不一致[" + frontUser.getOwnerId());
...
//		}
		...
		logger.debug("我不信："+map.get("documentTypeId"));
		DocumentType documentType = documentTypeService.select(document.getDocumentTypeId());
		if(documentType == null){
		...
		if (documentType.getDataDefineMap() == null || documentType.getDataDefineMap().size() == 0) {
			...
			String autoTagConfig = configService.getValue(DataName.autoTagList.toString(), frontUser.getOwnerId());
			...
			for (DataDefine dataDefine : documentType.getDataDefineMap().values()) {
				...
				documentData.setOwnerId(frontUser.getOwnerId());
		...
		document.setCurrentStatus(130002);
		int insertRs = 0;
		try {
		...
		message = new EisMessage(OperateResult.success.getId(), "上传成功,请等待管理员审核");

		map.put("udid",document.getUdid());

		map.put("message", message);
		return "content/friend/upload/index";
	...
	public String update(HttpServletRequest request, HttpServletResponse response, ModelMap map,
			@ModelAttribute("document") Document document) throws Exception {
		long ownerId = 0;
			...
			for(DataDefine dataDefine: documentType.getDataDefineMap().values()){
				...
				if (documentDataStr == null || documentDataStr.equals("")) {
					...
					if(_oldDocument.getDocumentDataMap() != null){
						...
						if(dd != null && dd.getDataValue().equals(documentDataStr)){
