...
public class BuyController extends BaseController{
	...
	//向购物车添加一个商品之后的view
	final String addView = "cart/list";
	//为结算订单选择配送地址的view
	final String deliveryView = "buy/delivery";
	//提交结算的view，在这里进行支付，如显示微信支付的二维码等
	final String settleUpView = "buy/settleUp";
	/**
	 * 支付请求后跳转post 页面
	 */
	final String postView = "buy/post";
	...
	public String add(HttpServletRequest request, HttpServletResponse response, ModelMap map, String productCode, Item item) throws Exception {
		...
		if(StringUtils.isBlank(productCode)){
			logger.error("无法添加到购物车，因为没有提交任何产品代码");
			map.put("message", new EisMessage(EisError.OBJECT_IS_NULL.getId(),"请选择一个商品"));
			return frontMessageView;
		}
		...
		if(product.getCurrentStatus() != BasicStatus.normal.getId()){
			logger.error("请求购买的产品[" + productCode + "]状态异常:" + product.getCurrentStatus());
			map.put("message", new EisMessage(EisError.statusAbnormal.getId(), "找不到产品[" + productCode + "]"));
			return frontMessageView;		
		}
		if(product.getOwnerId() != frontUser.getOwnerId()){
			logger.error("请求购买的产品[" + productCode + "]ownerId[" + product.getOwnerId() + "]与用户的[" + frontUser.getOwnerId() + "]不一致");
			map.put("message", new EisMessage(EisError.statusAbnormal.getId(), "找不到产品[" + productCode + "]"));
			return frontMessageView;		
		}
		int availableCount = productService.getAmount(product.getProductId());
		logger.debug("尝试购买的产品[" + product.getProductId() + "]数量是:" + availableCount);
		if(availableCount <= 0 && availableCount != -1 ){
			logger.error("尝试购买的产品[" + productCode + "]数量为:" + availableCount);
			map.put("message", new EisMessage(EisError.stockEmpty.getId(), "产品[" + productCode + "]无货"));
			return frontMessageView;		
		}
		...
		} else if(product.getSupplyPartnerId() > 0){
			inviter = product.getSupplyPartnerId();
			logger.debug("前端用户[" + frontUser.getUuid() + "]没有推荐来源，使用产品[" + product.getProductId() + "]的供应商:" + inviter + "作为推广来源");
		} else {
			logger.debug("前端用户[" + frontUser.getUuid() + "]没有推荐来源，产品[" + product.getProductId() + "]也没有供应商，不设置推广来源");
		}
		if(inviter > 0){
		...
		if(product.getProductDataMap() != null){
			logger.debug("产品[" + product.getProductId() + "]的扩展数据有[" + product.getProductDataMap().size() + "]个");

			for(ProductData productData : product.getProductDataMap().values()){
				ProductData itemData = productData.clone();
				if(StringUtils.isBlank(itemData.getDataValue())){
					logger.info("忽略产品空的扩展数据:" + itemData.getDataCode() + "=" + itemData.getDataCode() + "/" + itemData.getDataValue());  

				} else {
					itemData.setProductDataId(0);
					item.getItemDataMap().put(itemData.getDataCode(), itemData);
					logger.info("放入产品的扩展数据:" + itemData.getDataCode() + "=" + itemData.getDataCode() + "/" + itemData.getDataValue() + "到订单[" + item.getTransactionId() + "]");  
				}
		...
		if(StringUtils.isNotBlank(transactionToken)){
			//price = priceService.getPriceByToken(product.getProductId(), buyUuid, transactionToken);
		}
		if(price == null){
		...
		if(refDocument != null){
			...
			item.setExtraValue(DataName.refImage.toString(), refDocument.getExtraValue(DataName.productSmallImage.toString()));
		}
		...
		if(cart != null){
			...
			if(!ignoreDelivery){
				String fromArea = null;//deliveryOrderService.getFromArea(product);
				if(fromArea != null){
	...
	public String  delivery(HttpServletRequest request, HttpServletResponse response, ModelMap map			) throws Exception {
		...
		Money money = moneyService.select(buyUuid,ownerId);
		if(money != null){
			map.put("money", money);
		}
		...
		for(Item item : submitCart.values()){
			if(item != null){
				...
				if(product.getProductDataMap() == null){
					logger.error("订单[" + item.getTransactionId() + "]对应产品[" + item.getProductId() + "]没有扩展数据");
				} else {
					for(ProductData pd : product.getProductDataMap().values()){
						logger.info("产品[" + product.getProductId() + "]扩展数据:" + pd.getDataCode() + "=>" + pd.getDataValue());
					}
				}
				//检查该交易是否需要送货
		...
		if(order == null){
			...
			map.put("message", new EisMessage(EisError.REQUIRED_PARAMETER.getId(), "您选择购物车为空"));
			return frontMessageView;
		}
	...
	public String  settleUp(HttpServletRequest request, HttpServletResponse response, ModelMap map
		...
		for(Item item : submitCart.values()){
			if(item != null){
				...
				if(!ignoreDelivery){
					//检查该交易是否需要送货
					boolean needDelivery = product.getBooleanExtraValue(DataName.productNeedDelivery.toString());
					logger.debug("订单[" + item.getTransactionId() + "]对应的产品[" + product.getProductId() + "]" + (needDelivery ? "" : "不") + "需要配送");
					if(needDelivery){
						if(addressBookId == 0){
							logger.info("订单[" + item.getTransactionId() + "/" + item.getProductId() + "]需要配送，但未提交配送地址");
							toSelectDelivery = true;
					...
					boolean selfCarry = product.getBooleanExtraValue(DataName.selfCarry.toString());
					logger.debug("订单[" + item.getTransactionId() + "]对应的产品[" + product.getProductId() + "]" + (selfCarry ? "" : "不") + "支持自提");
					if(selfCarry){
						if(addressBookId == 0){
							logger.info("订单[" + item.getTransactionId() + "/" + item.getProductId() + "]支持自提，但未提交自提地址");
							toSelectSelfCarry = true;
		...
		for(Item t : submitCart.values()){
			...
			Product product = productService.select(t.getProductId());
			if(product == null){
				logger.error("找不到购物车中的商品:" + t.getProductId());
				map.put("message", new EisMessage(EisError.productNotExist.getId(),"找不到指定的产品"));
				return settleUpView;	
			}
			int availableCount = productService.getAmount(t.getProductId());
			if(availableCount == 0){
				logger.error("商品:" + product.getProductId() + "/" + product.getProductCode() + "]数量为0，无法购买");
			...
			if(availableCount != -1){
				...
				logger.warn("商品[" + product.getProductId() + "/" + product.getProductCode() + "]数量是-1，视为无数量限制");
			}
			String validatorClass = product.getExtraValue(DataName.productValidator.toString());
	...
	public String  countDeliveryFee(HttpServletRequest request, HttpServletResponse response, ModelMap map
		...
		if(submitCart == null || submitCart.size() < 1){//所选订单异常
			map.put("message", new EisMessage(EisError.OBJECT_IS_NULL.getId(), "选择订单异常"));
			return frontMessageView;
		}
		Item[] items = new Item[submitCart.size()];
		items = submitCart.values().toArray(items);
		...
		if(addressBookId > 0){
			...
			if(addressBook == null){
				...
				map.put("message", new EisMessage(EisError.OBJECT_IS_NULL.getId(), "找不到对应的地址本"));
				return frontMessageView;
			}
	...
	public String setInvoice(HttpServletRequest  request, HttpServletResponse response, ModelMap map, 		
		...
		if(cart == null){
			...
			map.put("message",new EisMessage(OperateResult.failed.getId(),"请提供正确的orderId"));
			return frontMessageView;
		...
		if(invoiceList == null || invoiceList.size() < 1){
			...
			if(rs == 1){
				...
				map.put("message",new EisMessage(OperateResult.failed.getId(),"新增发票失败"));
				return frontMessageView;
			}
		...
		if(invoiceInfo == null){
			map.put("message",new EisMessage(OperateResult.failed.getId(),"设置发票失败"));
			return frontMessageView;
		}
		map.put("order", cart);
	...
	public String setCouponMoney(HttpServletRequest request, HttpServletResponse response, ModelMap map) throws Exception {
		...
		if(order == null){
			logger.error("找不到指定的购物车[" + orderId + "]");
			map.put("message", new EisMessage(EisError.cartIsNull.getId(),"系统数据异常"));
			return frontMessageView;	
		}
		...
		if(couponIds == null){
			...
			map.put("message", new EisMessage(EisError.REQUIRED_PARAMETER.getId(),"您当前的订单没有使用优惠券"));
			return frontMessageView;	
		}
		...
		if(useMoney > totalCouponMoney){
			...
			map.put("message", new EisMessage(EisError.REQUIRED_PARAMETER.getId(),"您添加的优惠券总金额是" + totalCouponMoney + ",不足" + useMoney));
			return frontMessageView;	
		}
		...
		order.getData().put(DataName.askUseCouponMoney.toString(), String.valueOf(useMoney));
	...
	private String _setCoupon(Map<String, String> requestDataMap, User frontUser,	ModelMap map, String urlPrefix){
		...
		if(couponModelList == null || couponModelList.size() < 1){
			...
			map.put("message", new EisMessage(EisError.REQUIRED_PARAMETER.getId(),"找不到指定的卡券产品:" + couponCode));
			return frontMessageView;	
		}
		...
		if(couponModel.getProcessor() == null){
			...
			map.put("message", new EisMessage(EisError.beanNotFound.getId(),"指定的卡券产品数据异常"));
			return frontMessageView;
		} 
	...
	public String setDeliveryAddress(HttpServletRequest  request, HttpServletResponse response, ModelMap map			) throws Exception {
		...
		if(addressBookId < 1){
			map.put("message", new EisMessage(EisError.REQUIRED_PARAMETER.getId(), "请提交地址ID"));
			return frontMessageView;
		}
