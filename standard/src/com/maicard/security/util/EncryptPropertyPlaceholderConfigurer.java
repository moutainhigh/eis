...
public class EncryptPropertyPlaceholderConfigurer extends PropertyPlaceholderConfigurer{
	...
	private boolean needCrypt(String key){
		for(String needKey : needEncryptNames){
			if(key.toLowerCase().indexOf(needKey.toLowerCase()) >= 0){
				return true;
			}
		}
		return false;
	...
	protected String convertProperty(String propertyName, String propertyValue) { 
		if(securityLevel <= SecurityLevelCriteria.SECURITY_LEVEL_NORMAL){
	...
	private String decrypt(String propertyValue) {
		...
		try{
			return crypt.des3Decrypt(propertyValue);
		}catch(Exception e){
