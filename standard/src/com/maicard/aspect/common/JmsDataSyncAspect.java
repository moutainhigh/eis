...
import com.maicard.common.util.ImplNameTranslate;
...
public class JmsDataSyncAspect extends BaseService{
	...
	@Around("(execution(* com.maicard..service..*.update*(..)) " 
			+ "|| execution(* com.maicard..service..*.insert*(..)) " 
			+ "|| execution(* com.maicard..service..*.delete*(..)) " 
			+ "|| execution(* com.maicard..service..*.move*(..)) " 
			+ "|| execution(* com.maicard..service..*.sync*(..)) " 
			+ "|| execution(* com.maicard..service..*.load(..)) " 
			+ "|| @annotation(com.maicard.annotation.NeedJmsDataSync)) && (!@annotation(com.maicard.annotation.IgnoreJmsDataSync))")
...
*/		Signature  signature = joinPoint.getSignature();
		...
		if(eisMessageListener != null){
			if(logger.isDebugEnabled()){
				logger.debug("目标服务是一个EisMessageListener实现，JMS动态更新由其自身完成，忽略");
			}
			return result;
		}
		if(params != null && params.length > 0 && params[0] != null){		
			logger.debug("第一个参数类型是[" + params[0].getClass().getName() + "]");
			if(params[0] instanceof JmsObject){
				eisParameters = (JmsObject)params[0];
			}
		} else {
		...
		if(eisParameters == null && !isDelete){
			logger.debug("参数类型既不是JmsObject，执行方法也不是delete开头，忽略");
			return result;
		}*/
		boolean jmsDataSyncFromAll = false;
		if(eisParameters != null && eisParameters.getClass().isAnnotationPresent(NeedJmsDataSyncP2P.class) ){
		...
		if(!isSomeMasterHandler && !jmsDataSyncFromAll){
			if(logger.isDebugEnabled()){
				logger.debug("本节点不是任何一种数据处理节点，对象也没有NeedJmsDataSyncFromAll注解，忽略");
			}
			return result;		
		}
		...
		if(result instanceof EisMessage){
			rs = ((EisMessage)result).getOperateCode();
		...
		if(execSucess){
			...
			syncMessage.getAttachment().put("updateSlaveParamaters", params);
			...
			try{
				...
				/*if(method.isAnnotationPresent(CacheEvict.class)){
					CacheEvict e = method.getAnnotation(CacheEvict.class);
					String[] cacheNames = e.value();
					if(cacheNames == null || cacheNames.length < 1){
						logger.error("错误的注解值，没有指定缓存名称cacheName");
					}
					String cacheKey = e.key();
					logger.debug("切入自定义缓存处理, bean：" + joinPoint.getTarget().getClass().getSimpleName() + ", method:" +joinPoint.getSignature().getName() + "具有CacheEvict注解,注解值:" + cacheKey); 
					String key = parseKey(cacheKey, method, joinPoint.getArgs());
					logger.debug("解析后的KEY=" + key);
					for(String cacheName : cacheNames){
						logger.debug("发送cacheService.deleteWithCacheNameAndKey(" + cacheName + "#" + key + "");
						messageService.sendJmsDataSyncMessage(null, "cacheService", "deleteWithCacheNameAndKey", cacheName + "#" + key);
					}
				} */
				syncMessage = null;
			}catch(Exception e){
