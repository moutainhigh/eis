...
import com.maicard.mb.criteria.EsSessionCriteria;
...
import com.maicard.standard.SessionStatus;

import static com.maicard.standard.CommonStandard.ES_SESSION_MAP_NAME;
/**
...
public class EsSessionServiceImpl extends BaseService implements EsSessionService{
	...
	@Override
	public  long count(EsSessionCriteria esSessionCriteria){
	...
	public int sendLocalMessageDirect(long uuid, Object value, boolean forceSend){
		...
		synchronized(s){
			try {
				...
				logger.info("直接向本地Session发送消息完成，摘要:{},时间戳:{}", (value.toString().length() > CommonStandard.MESSAGE_BRIEF_LENGTH ? value.toString().substring(0, CommonStandard.MESSAGE_BRIEF_LENGTH) + "......" : value), System.currentTimeMillis());
			} catch (IOException e) {
	...
	public  int sendMessage(ModelMap map, long... uuids){
		...
		logger.info("向用户{}发送消息，摘要:{},时间戳:{}", Arrays.toString(uuids), (value.length() > CommonStandard.MESSAGE_BRIEF_LENGTH ? value.substring(0, CommonStandard.MESSAGE_BRIEF_LENGTH) + "......" : value), System.currentTimeMillis());
		for(long uuid : uuids){
	...
	public void addMessageToQueue(long uuid, Object value, long score){
		String key = KeyConstants.USER_MQ_INSTANCE_PREFIX + "#" + uuid;
	...
	public void clearOldSessionForUser(EsSession wsSession) {
		...
		Set<Object> keys = centerDataService.getHmKeys(ES_SESSION_MAP_NAME);
	...
	public void handlerMessageQueue(EsSession wsSession) throws Exception{
		...
		String monitorKey = KeyConstants.USER_MQ_LOCK_PREFIX + "#" + uuid;
		...
		while(true){
			...
			if(wsSession.getCurrentStatus() != SessionStatus.READY_FOR_MESSAGE.id){
				//还没有准备好接受消息
				Thread.sleep(CommonStandard.USER_MQ_READ_INTERVAL);
				continue;
			}
			...
			String key = new StringBuffer().append(KeyConstants.USER_MQ_INSTANCE_PREFIX).append("#").append(wsSession.getUser().getUuid()).toString();
	...
	@Override
	public List<EsSession> listOnPage(EsSessionCriteria esSessionCriteria) {
