 ...
 * @date 2013-9-16 
 */
public class JmsDataSyncServiceImpl extends BaseService implements JmsDataSyncService {
	...
	public void operate(EisMessage eisMessage) {
		...
		boolean runBothServer = execOnBothNode != null ? true : false;
		if(handlerJmsDataSyncToLocal || runBothServer){
			...
			try {
				execMethod.invoke(bean, paraArray);
				executed = true;
			} catch (Exception e) {
				logger.error("在调用服务[" +beanName + "]执行方法[" + methodName + "],参数[" + parameters + "]时出错:" + e.getMessage());
				logger.error(ExceptionUtils.getFullStackTrace(e));
			}

			if(executed){
				logger.debug("服务[" + beanName + "],方法[" + methodName + "]已完成调用");
			} else {
	...
	private boolean isCenterCache() {
		String cacheName = CommonStandard.cacheNameSupport;
