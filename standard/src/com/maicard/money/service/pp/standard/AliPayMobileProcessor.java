...
/**
 * 
 * 更新为新版接口，支付宝页面会自动尝试调用支付宝APP，基本类似于原生调用,NetSnake,2016-10-20
 * 
 * 
 * 支付宝无线/WAP充值接口
 * @author NetSnake
 ...
 */
@Service
public class AliPayMobileProcessor extends BaseService implements PayProcessor {
	...
	public EisMessage onPay(Pay pay){
		...
		//PayChannelMechInfo alipayMechInfo = getMechInfo(pay.getOwnerId());
		...
		if(pay.getContextType() != null && pay.getContextType().equalsIgnoreCase(ContextType.APP.toString())){
		...
		if(StringUtils.isNotBlank(notifyUrl)){
			...
			sParaTemp.put("show_url", notifyUrl);
		}
...
*/
	...
	private PayChannelMechInfo getMechInfo(long ownerId) {
		...
		if(alipayMechInfo == null){		
			...
			String key = configService.getValue(DataName.alipayAccountId.toString(), ownerId);
			if(key != null){
				alipayMechInfo.accountId = key;
			} else {
				logger.error("找不到ownerId=" + ownerId + "的支付宝信息alipayAccountId");
				return null;
			}

			String accountName = configService.getValue(DataName.alipayUserName.toString(), ownerId);
			if(accountName != null){
				alipayMechInfo.accountName = accountName;
			} else {
				logger.error("找不到ownerId=" + ownerId + "的支付宝信息alipayUserName");
				return null;
			}
			String payKey = configService.getValue(DataName.alipayKey.toString(), ownerId);
			if(payKey != null){
				alipayMechInfo.payKey = payKey;
			} else {
				logger.error("找不到ownerId=" + ownerId + "的支付宝信息payKey");
				return null;
			}		
			synchronized(this){
	...
	private String verifyResponse(String notify_id, long ownerId) {
		/*String accountId = null;
		String payUrl = null;
		try{
			accountId = alipayPartner.getUserConfigMap().get(DataName.supplierAgentId.toString()).getDataValue();
			payUrl = alipayPartner.getUserConfigMap().get(DataName.supplierChargeUrl.toString()).getDataValue();

		}catch(NullPointerException e){
			throw new RequiredObjectIsNullException("找不到指定的支付通道数据");		

		}*/
		//获取远程服务器ATN结果，验证是否是支付宝服务器发来的请求

		PayChannelMechInfo alipayMechInfo = getMechInfo(ownerId);

		String veryfy_url = payUrl  + "service=notify_verify&partner=" + alipayMechInfo.accountId + "&notify_id=" + notify_id;
		System.out.println(veryfy_url);
		return HttpUtils.sendData(veryfy_url);
	}
...
*/
	...
	public String getDesc() {
		return "支付宝移动支付处理器" + this.getClass().getSimpleName();
	}
