package com.maicard.money.service.standard;
 ...
 */
@Service
public class ChannelRobinServiceImpl extends BaseService implements ChannelRobinService {
	@Resource
	...
	public PayMethod getPayMethod(Pay pay, User partner) {
		...
		boolean internal = pay.getBooleanExtraValue("internal");

		/* 根据payType的ID和contextType来确定哪一种payMethod */
		PayMethodCriteria payMethodCriteria = new PayMethodCriteria(pay.getOwnerId());
		payMethodCriteria.setPayTypeId(pay.getPayTypeId());
		payMethodCriteria.setContextType(pay.getContextType());

		if(internal){
			payMethodCriteria.setCurrentStatus(BasicStatus.hidden.getId());
		} else {
			payMethodCriteria.setCurrentStatus(BasicStatus.normal.getId());
		}

		int[] excludePayMethods = null;
		String exclude = pay.getExtraValue("excludePayMethod");
		...
		if(payMethodList == null || payMethodList.size() < 1){
			logger.error("根据[payTypeId:" + pay.getPayTypeId() + ",contextType:" + pay.getContextType() + ",ownerId=" + payMethodCriteria.getOwnerId() + "]找不到任何支付方式");
			return null;
		}
		...
		if(pay.getPayFromAccountType() == UserTypes.partner.getId() && partner != null){
			limitPayToChannelOnly = partner.getBooleanExtraValue("limitPayToChannelOnly");
			perferChannelId = (int)partner.getLongExtraValue("perferChannelId");
		}
		...
		/*if(pay.getContextType() == null){
			logger.debug("根据[支付类型:" + pay.getPayTypeId() + ",contextType:" + pay.getContextType() + "]返回的支付环境未指定，直接返回第一个支付方式:" + payMethodList.get(0));
			return payMethodList.get(0);
		}*/
		//
		//int highestWeight = payMethodList.get(0).getWeight();
		...
		for(PayMethod payMethod: payMethodList){			
			...
			//判断这一个小时的成功金额是否已经超限
			String hourSuccessKey = "PayMethod#SuccessMoney#" + hour + "#" + payMethod.getPayMethodId();
			float hourSuccessMoney = NumericUtils.parseFloat(centerDataService.get(hourSuccessKey));
			float maxSuccessMoneyInHour = payMethod.getFloatExtraValue("maxSuccessMoneyInHour");
			logger.debug("支付方式[" + payMethod.getPayMethodId() + "]本小时成功金额" + hourSuccessMoney + "+订单[" + pay.getTransactionId() + "]金额:" + pay.getFaceMoney() + ",最大限定金额是:" + maxSuccessMoneyInHour + "");
			...
			String daySuccessKey = "PayMethod#SuccessMoney#" + day + "#" + payMethod.getPayMethodId();
			float daySuccessMoney = NumericUtils.parseFloat(centerDataService.get(daySuccessKey));
			float maxSuccessMoneyInDay = payMethod.getFloatExtraValue("maxSuccessMoneyInDay");
			logger.debug("支付方式[" + payMethod.getPayMethodId() + "]当天成功金额" + daySuccessMoney + "+订单[" + pay.getTransactionId() + "]金额:" + pay.getFaceMoney() + ",最大限定金额是:" + maxSuccessMoneyInDay + "");
			...
			float minPayMoney = payMethod.getFloatExtraValue("minPayMoney");
			if(minPayMoney > 0 && pay.getFaceMoney() < minPayMoney){
				logger.info("当前订单支付金额:" + pay.getFaceMoney() + ",小于当前支付方式[" + payMethod.getPayMethodId() + "]的最小订单金额:" + minPayMoney + ",不再使用");
				continue;
			}
			boolean noWapDirect = payMethod.getBooleanExtraValue("noWapDirect");
			if(noWapDirect && wapDirect){
				logger.info("当前订单要求使用WAP直接跳转，但当前支付方式[" + payMethod.getPayMethodId() + "]不支持该方式，不再使用");
				continue;
			}
			float maxPayMoney = payMethod.getFloatExtraValue("maxPayMoney");
			if(maxPayMoney > 0 && pay.getFaceMoney() > maxPayMoney){
				logger.info("当前订单支付金额:" + pay.getFaceMoney() + ",大于当前支付方式[" + payMethod.getPayMethodId() + "]的最大订单金额:" + minPayMoney + ",不再使用");
				continue;
			}
			if(excludePayMethods != null && excludePayMethods.length > 0){
				boolean shouldExclude =false;
				for(int ex2 : excludePayMethods){
					if(payMethod.getPayMethodId() == ex2){
		...
		if (ChannelRobinServiceImpl.ROBIN_TYPE_GLOBAL.equals(channelRobinType)) {
			usePayMethod = this.roundChannel(payMethodList2, ChannelRobinServiceImpl.ROBIN_TYPE_GLOBAL,ChannelRobinServiceImpl.ROBIN_TYPE_GLOBAL, String.valueOf(pay.getPayTypeId()));
		} else if (ChannelRobinServiceImpl.ROBIN_TYPE_MERCHANT.equals(channelRobinType)) {
			usePayMethod = this.roundChannel(payMethodList2, ChannelRobinServiceImpl.ROBIN_TYPE_MERCHANT,String.valueOf(pay.getPayFromAccount()), String.valueOf(pay.getPayTypeId()));
		} else if (ChannelRobinServiceImpl.ROBIN_TYPE_FIFO.equals(channelRobinType)) {
	...
	public PayMethod fifoUseChannelRobin(List<PayMethod> payMethodList2) {
		...
		if(payMethodList2.size() < 2) {
			...
			for(PayMethod payMethod : payMethodList2) {
				key = keyPrefix + "#" + payMethod.getPayMethodId();
		...
		if(selectPayMethod == null) {
			selectPayMethod = payMethodList2.get(0);
			logger.warn("无法根据最早规则得到payMethod，使用第一个payMethod:{}", selectPayMethod.getPayMethodId());
		}
		key = keyPrefix + "#" + selectPayMethod.getPayMethodId();
		centerDataService.setForce(key, StringTools.getFormattedTime(new Date()), ttl);
		logger.debug("本次根据使用时间FIFO原则选择的payMethod是:{}",selectPayMethod.getPayMethodId());
