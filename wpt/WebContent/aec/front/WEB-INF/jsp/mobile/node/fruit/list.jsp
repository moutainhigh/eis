...
<link rel="stylesheet" type="text/css" href="../../../../css/mobile/index.css?v=0701121438"/>
	...
	<%@include file="/WEB-INF/jsp/include/header.jsp" %>
		  <div class="navbar navbar1 mt5">
		     <p><span class="orange">	 <c:choose>
				<c:when test="${node.name=='生鲜果蔬'}">
					•生•鲜•果•蔬•
				</c:when>
				<c:when test="${node.name=='南北干货'}">
					•南•北•干•货•
				</c:when>
				<c:when test="${node.name=='休闲食品'}">
					•休•闲•食•品•
				</c:when>
				<c:otherwise>
					•溯•食•同•源•
		  ...
		  <c:when test="${fn:length(newsList)>0}">
		   <c:forEach items="${newsList}" var="document">
				...
				<img data-original="${document.documentDataMap.get('productSmallImage').dataValue}" src="../../../../image/mobile/listImgDefault.png" class="lazy"/>
				<div class="p-info">
					<div class="left">
					  <p>${document.title}</p>
