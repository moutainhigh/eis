...
<link rel="stylesheet" type="text/css" href="../../../css/mobile/searchList.css" />
...
<body>
	...
	<!-- 搜索结果 -->
    <div class="box_left">
		<p class="mb30"><span class="fc">以先共为您搜索到相关结果${totalRecord }个</span></p>
		</div>
		<c:forEach items="${result.searchList }" var="result">
			...
			<c:if test="${!empty result.unitPrice }">
				<div class="box_left">
					<a href="${result.url }">
				   	<ul class="box_product martop45 bor">
						 <img src="${result.simages }"/>
					  	
					  		<div class="commodity">
					  		<p style="font-size:18px;">${result.title }</p>
                            <c:set value="${ fn:split(result.origin, '#') }" var="str1" />
					  		<p class="mt4">产地：<!--${result.origin }-->${str1[0] }&nbsp&nbsp&nbsp${str1[1] }</p>
					  		<p class="mt4">规格：${result.specifications }</p>
					  		<p class="mt4">单价：<span class="orange">￥${result.unitPrice }</span></p>
			...
			<c:if test="${empty result.unitPrice }">
	    		<c:if test="${empty result.simages2 }">
	    			<div class="box_left">
						<p class="martop45 font-title"><a href="${result.url }">${result.title }</a></p>
				   ...
				   <div class="fc martop10">2016-05-19 11:33 &nbsp;&nbsp;&nbsp;[${result.plateName }]</div>
					...
					<c:otherwise>
						...
						<ul class="article">
			    			<li>
			       				<img src="${result.simages2 }">
			    				<div class="message"><!--${result.content }-->${fn:substring(result.content,0,50)}${fn:length(result.content)>50?"...":""}</div> 
							</li>
						</ul>
						<div class="fc martop10">${result.createTime } &nbsp;&nbsp;&nbsp;[${result.plateName }]</div>
						<hr style="border: 1px #cfcfcf solid;" />
