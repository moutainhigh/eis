	...
	<div id="bigView" style="width: 313.856427378965px; height: 229.425px; top: 180px; left: 1064.5px; display: none;"><img alt="" width="800" height="800" src="../../../../image/product/img1.jpg" style="left: 0px; top: -122.36px;"></div>
        ...
        <div class="box_right">
			...
			<div class="ware_sprice" style="padding-left:15px;">
			   <p class="fo-14">以先推荐售价：<span class="red">￥<span class="fo-26 fo_mA">${document.documentDataMap.get('activityPrice').dataValue}</span></span></p>
			   <p class="fo-12">市场产品售价价：<span>￥<span class="fo_mA">${document.documentDataMap.get('marketPrice').dataValue}	</span></span></p>
			</div>
			...
			<div class="count fo pa8">
				<span class="text_left">活动时间：</span>
				<fmt:parseDate value="${document.documentDataMap.activityBeginTime.dataValue}" var="beginTime"/>
                 ...
                 <fmt:formatDate value="${beginTime}" type="date"/>—<fmt:formatDate value="${endTime}" type="date"/>
			</div>
			<div class="count fo pa8 colockbox" id="colockbox1"> 活动倒计时：<span class="data_space day orange">00</span>天<span class="data_space hour orange">00</span>时<span class="data_space minute orange">00</span>分<span class="data_space second orange">00</span>秒 </div>
			<div class="count fo pa8">
				<span class="text_left">产品份数：</span>
			   ...
			   <span class="data_space">${document.documentDataMap.get('soldCount').dataValue}/${document.documentDataMap.get('availableCount').dataValue + document.documentDataMap.get('soldCount').dataValue}</span>
			</div>
			<div class="count fo pa8">
				<span class="text_left">发货规则：</span>活动结束后三天内陆续原产地发货
			</div>
            <div class="ware_format   pa8 fo">发货地：${document.documentDataMap.get("deliveryFromArea").dataValue} ${document.documentDataMap.get("deliveryDesc").dataValue}</div>
            <input type="hidden" value="${product.transactionToken}" id="transactionToken"/>
			 <c:choose>
      			<c:when test="${result.operateCode==500080}">
				<div class="btnG martop50 pa ">
					<input type="button" value="即将开售" class="btn_buy_gray">
				</div> 
	  			</c:when> 
				<c:when test="${result.operateCode==500072}">
				<div class="btnG martop50 pa ">
					<input type="button" value="已结束" class="btn_buy_gray">
				</div> 
	  			</c:when> 
				<c:when test="${document.documentDataMap.get('availableCount').dataValue=='0'&&result.operateCode!=500080&&result.operateCode!=500072}">
				<div class="btnG martop50 pa ">
					<input type="button" value="已售罄" class="btn_buy_gray">
				</div> 
	  			</c:when> 
				<c:otherwise> 
				<div class="btns btnG martop50  pa8">
					<input type="button" value="加入购物车" class="add_cart" id="add_cart"/>
					<input type="button" value="立即购买" class="btn_buy" id="btn_buy" onclick="buynow('${result.attachment.transaId}')"/>
				</div> 
	        </c:otherwise>
            </c:choose>
			 <c:if test="${result.operateCode==500072}">
				<img src="../../image/icon_over.png" class="icon_over"/>
		     </c:if >
			...
			<div class="viewed martop302">
			 ...
			 <div class="viewDiv martop20">
			  ...
			  <c:when test="${!empty c.data.userHeadPic}">
				...
				<!-- <p class="martop15" id="tel" >${c.data.userRealName} </p> -->
				...
				<!-- <c:out value="${fn:substring(c.data.userRealName,0,6)}****"></c:out> -->
				</p>
				 </div>
				  <div class="view_con">
