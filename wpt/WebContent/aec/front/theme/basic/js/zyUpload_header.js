		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				var div= '';
					...
					div += '<div class="dropbox box_container_left" id="dropbox"></div><div class="box_container_right"><p class="orange fo-14">拖进左边圆圈即可</p><a class="btnSave martop45 upload_submit_btn">保存</a></div></form>';
				
	           $(".box_imgload").append(div);
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					div = '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
					/*html += '		</div>';
					html += '	</a>';
					...
					html = '<div id="uploadList_'+ file.index +'" class="upload_append_list"><div class="file_bar"><div style="padding:5px;"><p class="file_name">'
					+ file.name + '</p>'+delHtml+'</div></div><a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="#" class="imgBox"><div class="uploadImg" style="width:'+imgWidth+'px">'			
					+'<img id="uploadImage_'+file.index+'" class="upload_image" src="' + fileImgSrc + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />'                                                            
					+'</div></a><p id="uploadProgress_'+file.index+'" class="file_progress"></p>'
					+'<p id="uploadFailure_'+file.index+'" class="file_failure">上传失败，请重试</p>'
					+'<p id="uploadSuccess_'+file.index+'" class="file_success"></p>'
					+'</div>'
				}
			...
			this.createCorePlug = function(){
				var params = {
					fileInput: null,
					uploadInput: $(".upload_submit_btn").get(0),
