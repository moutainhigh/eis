	...
	window[fnName] = function(magnifierAttr){
		...
		_this.eqImg = function(){
			...
			function imgLoadEnd(){
				...
				imgBox.empty().append('<img src="'+src+'" width="'+imgWidth+'" height="'+imgHieght+'" style="'+styleCss+';margin-top:55px;" />');
		...
		_this.moveFn = function(e){
			...
			//当Y轴超出容器
			// 定制：控制鼠标在图片上移动的范围
			endY = (endY > 0) ? (endY > (360)) ? (360) : endY : 0;
			// console.log(magnifier.width-boxMoveViewHeight);
			// console.log(magnifier.height-boxMoveViewHeight);
			_this.moveView.css({
				'left' : endX,
				'top' : endY,
				'display' : "block"
