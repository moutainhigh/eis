...
@Service
public class CookieServiceImpl extends BaseService implements CookieService {




	@Override
	public void addCookie(HttpServletRequest request, HttpServletResponse response, String cookieName,
		...
		if(cookies == null){
			...
			for(Cookie c : cookies){
				if(c.getName().equalsIgnoreCase(cookieName)){
					...
					logger.debug("在请求中找到了同名cookie[" + cookieName +"]，直接设置该Cookie[name=" + c.getName() + ",value=" + c.getValue() + ",domain=" + c.getDomain() + ",path=" + c.getPath() + ",ttl=" + c.getMaxAge() + ",httpOnly=" + c.isHttpOnly() + ", version=" + c.getVersion());			
					return;
				}
			}
		}
		...
		if(logger.isDebugEnabled()){
			logger.debug("在请求中未找到同名Cookie，添加" + (isDomainCookie ? "域名" : "主机") + "Cookie:" + ",name=" + cookie.getName() + ",value=" + cookie.getValue() + ",设置domain=" + domain + ", cookie domain:" + cookie.getDomain() + ",ttl:" + cookie.getMaxAge() + ",httpOnly:" + httpOnly);
		}
	...
	@Override
	public void removeCookie(HttpServletRequest request, HttpServletResponse response, String cookieName, String domain) {
