...
function onCheckAccount(obj) {
	...
	if (obj.value == "") {
		...
		$(obj).focus();
			...
			if (msg == true) {
					$(obj).parent().next(".regok").text("邮箱可以注册");
					...
					$(obj).parent().next(".regok").text("邮箱已被使用");
					$(obj).parent().next(".regok").css('color', '#ff0000');
					$(obj).parent().next(".regok").css({
						'background' : 'url(/style/cuo.png)',
						'background-repeat' : 'no-repeat',
...
function registerSuccess(responseText, statusText) {
	...
	console.log("注册返回信息是:" + JSON.stringify(responseText) + ",statusText=" + statusText);
