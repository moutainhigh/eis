...
$(document).ready(function () {
	...
	$("#btn").click(function () {
		...
		if (checkSubmitMobil() != false) {
			...
			$.ajax({
				type : "POST",
				url : userHost + '/user/findPassword/phone/submit.json',
				data : {
				username : mobile,
			},
				dataType : 'json',
	...
	$("#bind").click(function () {
		...
		$.ajax({
			...
			url : userHost + '/user/findPassword/phone/confirm/' + $("#confirm_code").val() + '.json',
			...
			success : function (data) {
				...
				if (data.message.operateCode == operateResultSuccess) {
					...
					jAlert("请重新设置新密码", '提示信息', function (r) {
					
					
					var url=userHost + '/content/help/editNewPasswordByPhone.shtml?sign=' + $("#confirm_code").val();
