...
DROP PROCEDURE IF EXISTS `part_money_lock_2`$$

CREATE DEFINER=`chaoka`@`%` PROCEDURE `part_money_lock_2`(wantLockMoney INT, minLockMoney INT, lockStatus INT, transactionType INT, timeoutOnly BOOLEAN)
BEGIN
  ...
  SET autocommit=0;
  SELECT item_id INTO citem_id FROM item WHERE  request_money >= minLockMoney AND request_money <= wantLockMoney AND current_status=lockStatus AND transaction_type_id=transactionType AND process_count<=max_retry AND IF (timeoutOnly=TRUE, (UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(enter_time))> ttl,'1=1') LIMIT 0,1;
  ...
  BEGIN START TRANSACTION;
    ...
    clock_global_unique_id, csupply_partner_id FROM item a WHERE a.item_id=citem_id AND request_money>=minLockMoney FOR UPDATE;
