...
DROP PROCEDURE IF EXISTS `part_money_lock`$$

CREATE DEFINER=`chaoka`@`%` PROCEDURE `part_money_lock`(wantLockMoney INT,lockStatus INT, transactionType INT, timeoutOnly BOOLEAN)
BEGIN
  ...
  DECLARE csuccess_money VARCHAR(50);
  ...
  SET autocommit=0;
  SELECT item_id INTO citem_id FROM item WHERE  request_money>=wantLockMoney AND current_status=lockStatus AND transaction_type_id=transactionType AND IF (timeoutOnly=TRUE, (UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(enter_time))> ttl,'1=1') LIMIT 0,1;
  BEGIN START TRANSACTION;
    ...
    a.label_money,a.rate,a.count,a.request_money,a.success_money,a.enter_time,a.ttl,a.close_time,a.current_status,a.extra_status,a.cart_id,
    ...
    ccharge_to_account, clabel_money, crate, ccount, crequest_money, csuccess_money, center_time, cttl, cclose_time, ccurrent_status, cextra_status, ccart_id, cmoney_source, 
    clock_global_unique_id, csupply_partner_id FROM item a WHERE a.item_id=citem_id AND request_money-wantLockMoney>=0 FOR UPDATE;
  ...
  SELECT citem_id item_id, ctransaction_id transaction_id, ctransaction_type_id transaction_type_id, cname NAME, ccontent content, cproduct_id product_id, ccharge_from_account charge_from_account, 
    ccharge_to_account charge_to_account, clabel_money label_money, crate rate, ccount COUNT, crequest_money request_money, csuccess_money success_money, center_time enter_time, cttl ttl, cclose_time close_time, ccurrent_status current_status, cextra_status, ccart_id cart_id, cmoney_source money_source, 
